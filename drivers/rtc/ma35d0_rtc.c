 ...
 * get time
 *****************************************************************************/
int ma35d0_rtc_get(struct udevice *dev, struct rtc_time *tmp)
{
	struct ma35d0_rtc_priv *priv = dev_get_priv(dev);
 ...
 * set time
 *****************************************************************************/
int ma35d0_rtc_set(struct udevice *dev, const struct rtc_time *tmp)
{
	struct ma35d0_rtc_priv *priv = dev_get_priv(dev);
	uint32_t val = readl(priv->base + REG_RTC_INIR);
	int ret;
 ...
 * reset rtc
 *****************************************************************************/
static int ma35d0_rtc_reset(struct udevice *dev)
{
	return 0; /* nothing to do */
}

static int ma35d0_rtc_probe(struct udevice *dev)
{
	struct ma35d0_rtc_priv *priv = dev_get_priv(dev);
	uint32_t val;
	struct clk clk;
...
static const struct rtc_ops ma35d0_rtc_ops = {
	.get = ma35d0_rtc_get,
	.set = ma35d0_rtc_set,
	.reset = ma35d0_rtc_reset,
};

static const struct udevice_id ma35d0_rtc_ids[] = {
...
U_BOOT_DRIVER(rtc_nuvoton) = {
	...
	.of_match = ma35d0_rtc_ids,
	.ops	= &ma35d0_rtc_ops,
	.priv_auto_alloc_size = sizeof(struct ma35d0_rtc_priv),
};
