...
struct ma35h0_wdt_priv {
	fdt_addr_t base;		/* WDT Register address */
	struct regmap *sys;		/* SYS base */
};

static void ma35h0_reg_lock(struct ma35h0_wdt_priv *priv)
{
	u32 el = current_el();
...
static int ma35h0_reg_unlock(struct ma35h0_wdt_priv *priv)
{
...
static int ma35h0_wdt_reset(struct udevice *dev)
{
	struct ma35h0_wdt_priv *priv = dev_get_priv(dev);
...
static int ma35h0_wdt_start(struct udevice *dev, u64 timeout_ms, ulong flags)
{
	struct ma35h0_wdt_priv *priv = dev_get_priv(dev);
	u32 reg = WDTEN | RSTEN;
	int lock;
...
static int ma35h0_wdt_stop(struct udevice *dev)
{
	struct ma35h0_wdt_priv *priv = dev_get_priv(dev);
	int lock;
...
static int ma35h0_wdt_probe(struct udevice *dev)
{
	struct ma35h0_wdt_priv *priv = dev_get_priv(dev);
...
static const struct wdt_ops ma35h0_wdt_ops = {
	.start = ma35h0_wdt_start,
	.stop = ma35h0_wdt_stop,
	.reset = ma35h0_wdt_reset,
};

static const struct udevice_id ma35h0_wdt_match[] = {
	{ .compatible = "nuvoton,ma35h0-wdt" },
	{ }
};

U_BOOT_DRIVER(ma35h0_wdt) = {
	.name = "ma35h0-wdt",
	.id = UCLASS_WDT,
	.of_match = ma35h0_wdt_match,
	.priv_auto_alloc_size = sizeof(struct ma35h0_wdt_priv),
	.probe = ma35h0_wdt_probe,
	.ops = &ma35h0_wdt_ops,
};
