...
struct ma35h0_crypto_priv {
	struct regmap	*sysreg;
	void    *reg_base;
};

static struct ma35h0_crypto_priv  _ma35h0_crypto;

static inline void nu_write_reg(u32 val, u32 off)
{
	writel(val, _ma35h0_crypto.reg_base + off);
}

static inline uint32_t nu_read_reg(u32 off)
{
	return readl(_ma35h0_crypto.reg_base + off);
}
 ...
 */
int ma35h0_aes_decrypt(u32 keynum, u32 src_addr,
...
static int ma35h0_crypto_bind(struct udevice *dev)
{
	/*
	 * Get the base address for Crypto from the device node
	 */
	_ma35h0_crypto.reg_base = (void *)devfdt_get_addr(dev);
	if (_ma35h0_crypto.reg_base == (void *)FDT_ADDR_T_NONE) {
...
static const struct udevice_id ma35h0_crypto_ids[] = {
	{ .compatible = "nuvoton,ma35h0-crypto" },
	{ }
};

U_BOOT_DRIVER(crypto_ma35h0) = {
	.name	= "ma35h0_crypto",
	.id	= UCLASS_NOP,
	.of_match = ma35h0_crypto_ids,
	.bind = ma35h0_crypto_bind,
	.priv_auto_alloc_size = sizeof(struct ma35h0_crypto_priv),
