...
/* MA35D0 I2C registers */
struct ma35d0_i2c_regs{
...
struct ma35d0_i2c_priv{
	struct ma35d0_i2c_regs *regs;
	struct clk clk;
};
...
void I2C_MasterRx(struct ma35d0_i2c_regs *regs, i2c_dev *dev, uint32_t u32Status)
{
...
void I2C_MasterTx(struct ma35d0_i2c_regs *regs, i2c_dev *dev, uint32_t u32Status)
{
...
static int  ma35d0_i2c_read(struct udevice *dev, uchar chip, u8 *addr,int alen, uchar *buffer, int len)
{
 	struct ma35d0_i2c_priv *priv = dev_get_priv(dev);
	struct ma35d0_i2c_regs *regs = priv->regs;
	int i;
	uint32_t u32Status;
...
static int  ma35d0_i2c_write(struct udevice *dev, uchar chip, u8 *addr,int alen, uchar *buffer, int len)
{
	struct ma35d0_i2c_priv *priv = dev_get_priv(dev);
	struct ma35d0_i2c_regs *regs = priv->regs;
	int i;
...
static int ma35d0_i2c_set_bus_speed(struct udevice *dev, unsigned int sp)
{
	struct ma35d0_i2c_priv *priv = dev_get_priv(dev);
	struct ma35d0_i2c_regs *regs = priv->regs;
	ulong clk_rate;
...
static int ma35d0_i2c_xfer(struct udevice *dev, struct i2c_msg *msg, int nmsgs)
{
	...
	if (dmsg->flags & I2C_M_RD)
		return ma35d0_i2c_read(dev, dmsg->addr, omsg->buf,
				  omsg->len, dmsg->buf, dmsg->len);
	else
		return ma35d0_i2c_write(dev, dmsg->addr, omsg->buf,
				   omsg->len, dmsg->buf, dmsg->len);

	return 0;
}

static int ma35d0_i2c_probe(struct udevice *dev)
{
	struct ma35d0_i2c_priv *priv = dev_get_priv(dev);
	struct ma35d0_i2c_regs *regs;
	fdt_addr_t addr;
	...
	priv->regs = (struct ma35d0_i2c_regs *)addr;
	regs = priv->regs;
...
static const struct dm_i2c_ops ma35d0_i2c_ops = {
	.xfer = ma35d0_i2c_xfer,
	.set_bus_speed = ma35d0_i2c_set_bus_speed,
};

static const struct udevice_id ma35d0_i2c_ids[] = {
...
U_BOOT_DRIVER(nuvoton_i2c) = {
	.name = "nuvoton-i2c",
	.id = UCLASS_I2C,
	.of_match = ma35d0_i2c_ids,
	.probe = ma35d0_i2c_probe,
	.priv_auto_alloc_size = sizeof(struct ma35d0_i2c_priv),
	.ops = &ma35d0_i2c_ops,
};
